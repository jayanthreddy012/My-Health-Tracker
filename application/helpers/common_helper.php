...
if(!function_exists('loadViewHelper')){
	function loadViewHelper($filepath,$data){			
		...
		$CI->load->view('layouts/menu');	
		$CI->load->view($filepath,$data);
		$CI->load->view('layouts/footer');	
	}
	
}
if(!function_exists('loginregisterhelper')){
	function loginregisterhelper($filepath,$data){			
		$CI =& get_instance();		
		$CI->load->view('layouts/header',$data);
		$CI->load->view('layouts/menu1');
		$CI->load->view($filepath,$data);
		$CI->load->view('layouts/footer');	
	}
	
}
if(!function_exists('logindoctorhelper')){
	function logindoctorhelper($filepath,$data){			
		$CI =& get_instance();		
		$CI->load->view('layouts/header',$data);
		$CI->load->view('layouts/doctor_menu');
		$CI->load->view($filepath,$data);
		$CI->load->view('layouts/footer');	
	}
